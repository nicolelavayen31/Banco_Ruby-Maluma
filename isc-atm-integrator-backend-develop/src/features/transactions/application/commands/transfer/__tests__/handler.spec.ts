...
describe('TransferHandler', () => {
    ...
    describe('transfer execution', () => {
        ...
        it('should pair bank_a and bank_b across debit and credit transactions', async () => {
            await createHandler();
            accountRepo.seed([
                BankAccount.Builder.setId('acc-from')
                    .setReference('FROM')
            ...
            const result = await handler.execute(
                makeCmd({ amount: 2500, source_bank: 'bank_a' }),
            );

            expect(result.data[0].sourceBank).toBe('bank_a');
            expect(result.data[1].sourceBank).toBe('bank_b');
            expect(result.data[0].correlationId).toBe(
                result.data[1].correlationId,
            );
        });
    });
