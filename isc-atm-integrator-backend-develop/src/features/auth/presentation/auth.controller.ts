...
export class AuthController {
    ...
    public async refresh(
        ...
        const cookieValue = cookies?.['__Host-refresh-token'];
        const refreshToken = typeof cookieValue === 'string' ? cookieValue : '';
    ...
    public async generateApiKey(
        @Body() commandData: GenerateApiKeyCommand,
        @Client() client: IAuthClient,
    ): Promise<GenerateApiKeyResponse> {
        const createdById = client.userId;
        if (!createdById) {
            throw new Error('Authenticated client is missing userId');
        }
