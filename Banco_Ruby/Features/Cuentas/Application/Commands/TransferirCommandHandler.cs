...
namespace BancoCenit.Features.Cuentas.Application.Commands
{
    ...
    public class TransferirCommandHandler : IRequestHandler<TransferirCommand, Result<OperacionResponse>>
    {
        ...
        public async Task<Result<OperacionResponse>> Handle(TransferirCommand command, CancellationToken cancellationToken)
        {
            ...
            var request = new TransferenciaRequest(command.NumeroCuentaOrigen, command.NumeroCuentaDestino, command.Monto);
            ...
            // Enviar correo de notificación transaccional asíncrono
            ...
            // Se obtienen los destinatarios de la configuración (separados por comas)
            ...
            // Se disparan los envíos en segundo plano en paralelo
            ...
            return Result.Ok(new OperacionResponse(mensaje, origen.Saldo));
        }
    }
}
